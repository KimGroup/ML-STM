...
cc	accumax controls step size - no improvement in nstep -> smaller step.
	...
	double precision accumax, pi, vx(ni,nsap+nval+4)
	integer cat(nsap+nval+4)
...
cc	Initialize the random numbers.
	...
	file3='traininglog_full_wd00nobia4disl.dat'
	file4='network_full_wd00nobia4disl.dat'
c	file3='traininglog_full_wd00nobia5difv.dat'
c	file4='network_full_wd00nobia5difv.dat'
...
c	file5='data3/data100batch_2dislA1307_differentsmooth1'
c	file5(47:66)='_sameCat2only_00.csv'
c	print*, file5
	...
	do 3 i = 1, ni
	do 3 j = 1, nsap+nval+4
	vx(i, j) = 0.0
3	continue

	file1='data3/data25batch4cats4dislA1307_00.csv'
c	file1='data3/data25batch_2dislA1307_differentsmooth1_00.csv'
	do 5 ifile = 0, nfile-1
	file2 = file1
	file2(34:34) = char(ichar(file2(34:34))+ifile/10)
	file2(35:35) = char(ichar(file2(35:35))+ifile-ifile/10*10)
c	file2(47:47) = char(ichar(file2(47:47))+ifile/10)
c	file2(48:48) = char(ichar(file2(48:48))+ifile-ifile/10*10)
	open(20, file=file2)
	do 35 k = 1, 100
...
c	file2(61:61) = char(ichar(file2(61:61))+ifile/10)
c	file2(62:62) = char(ichar(file2(62:62))+ifile-ifile/10*10)
	open(20, file=file2)
	do 135 k = 1, 100
140	continue
	itemp = itemp + 1
...
cc	Ready to update weights and biases.
	if((m .eq. mini .and. ncount .eq. 0 .and. rand() .lt. 0.2) .or.
     &	   (m .eq. mini .and. ncount .eq. 1 .and. rand() .lt. 0.1))then
	itemp = 4* rand() + 1
	if(itemp .gt. 4) itemp = itemp - 1
	itemp = itemp +nsap+nval
	else
	itemp = nsap* rand() + 1
...
258	continue
	...
	if(itemp .le. nsap+nval+2) then
	delta2(info) = delta2(info) - 1.0
	db2(info) = db2(info) - 1.0
	else
	do 259 i = 1, no
	delta2(i) = delta2(i) - 0.25
	db2(i) = db2(i) - 0.25
259	continue
	endif
cc	Back propagation.
